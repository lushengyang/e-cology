  ...
  if(outObject){
	if(n == 0){	 
		...
		if(outType == 2){                                               //判断流程代理的结束时间不能大于开始时间
          ...
          if(bohai==1) {getLeaveDays();}
          if(timeindex==1){ getbegintime(n,bObj,eObj,btObj,etObj,manager_,passnoworktime_);}
          if(timeindex==2){ getendtime(n,bObj,eObj,btObj,etObj,manager_,passnoworktime_);}
          if(taskid==1){ onShowBeginTime1(n,txtObj1,spanObj1,endDateObj1,spanEndDateObj1,timeObj1,timespanObj1,endtimeObj1,endtimespanObj1,workLongObj1);}
          if(taskid==2){ onShowEndTime1(n,txtObj2,spanObj2,beginDateObj2,spanBeginDateObj2,endtimeObj2,endtimespanObj2,timeObj2,timespanObj2,workLongObj2);}
          if(CustomQuery==1){
        	  if(frmmain.check_con[outValue2*1]){
...
function onShowDocsTime(spanname,inputname,isbodymand){
	...
	mdl2.style.top  = ((ttyp == "image") ? ttop + thei -300 : ttop + thei + 22 - 300)+"px";
