...
(function (jQuery) {
	jQuery.fn.e8Browser = function (options) {
		...
		if(!addBtnLabel){
			try{
				addBtnLabel = SystemEnv.getHtmlNoteName(82,readCookie("languageidweaver"));
			}catch(e){
				addBtnLabel = "新建";
			}
		}
					...
					for(var i=0;i<browserSpanValues.length && i<browserValues.length;i++){
						...
						if(!!browserSpanValue){
							...
							if(!browserSpanValue.match(/\s*a\s*\/\s*>/)){
								...
								browserSpanValue = "<a href='"+_linkUrl+"'"+((linkUrl.toLowerCase().indexOf("javascript:")!=-1||linkUrl=="#")?"":" target=_blank")+">"+browserSpanValue+"</a>";
							}
		...
		if((hasInput==true||hasInput=="true") && isMustInput!="0"){
			...
			window.setTimeout(function(){
				if(isSingle=="false"||isSingle==false){
					...
					browserBox.find("#out"+inputID+"div").perfectScrollbar();
				}
