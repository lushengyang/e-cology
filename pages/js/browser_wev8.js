	   ...
	   return {
			...
			hoverShowNameSpan:function(obj){
				jQuery(obj).hover(function(){
					jQuery(this).addClass("e8_showNameHoverClass");
					jQuery(this).children(".e8_delClass").css({
						"visibility":"visible",
						"opacity":1
					});
				},function(){
					jQuery(this).removeClass("e8_showNameHoverClass");
					jQuery(this).children(".e8_delClass").css({
						"visibility":"hidden"
					});
				});
			...
			_writeBackData:function(fieldid,isMustInput,data,_options){
				try{
					...
					if(data){
						...
						__callback(fieldid,fieldid,isMustInput,options.hasInput,options);
					}
				}catch(e){
					if(window.console)console.log(e);
				}
		    ...
		    showModalDialogCommon:function(e,url,linkUrl,fieldid,isSingle,isAdd,isMustInput,params,options){
				...
				if(zDialog){
					...
					if (datas){
						...
						__callback(name,fieldid,isMustInput,hasInput,options);
					}
				}
			},
			...
			e8formatInitData: function(_options){
				...
				window["formatInitData"+nameAndId] = function(){
					try{
				...
				if(span.width()<=0){
					window.changeFunctions.push(window["formatInitData"+nameAndId]);
				}else{
					window["formatInitData"+nameAndId]();
				}
			},
