...
var __weaverTableNamespace__ = (function(){
	...
	function refreshPreviewItems(){
		...
		var sumWidth = Math.round(parseInt(item.width.replace(/%/g,""))+parseInt(nextitem.width.replace(/%/g,"")));
		var _width =  parseInt(targetnow.attr("_width"));
		var width = Math.round(_width/xTableWidth*100);
		var nextwidth = sumWidth - width;
		if(nextwidth<4){
			return false;
		}
		//console.log(sumWidth+"::"+width+"::"+nextwidth);
		item.width =width+"%";
		cloneitem.width = width+"%";
		nextitem.width = nextwidth+"%";
		clonenextitem.width = nextwidth+"%";
		cloneitem.el.css("width",width+"%").attr("width",width+"%");
		clonenextitem.el.css("width",nextwidth+"%").attr("width",nextwidth+"%");
		item.el.css("width",width+"%").attr("width",width+"%");
		nextitem.el.css("width",nextwidth+"%").attr("width",nextwidth+"%");
		return true;
	}
