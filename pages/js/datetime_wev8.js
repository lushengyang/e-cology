...
function genDateAndWorkday(obj,begindate_,enddate_,workday_,begintime_,endtime_,manager,passnoworktime){
	if(obj&&begindate_&&enddate_&&workday_){
		...
		timeindex = "1";
		onProjTaskTime(begintimespan,btObj);
			 ...
			 timeindex = "2";
			 onProjTaskTime(endtimespan,etObj);
...
function getbegintime(returnvalue,bObj,eObj,btObj,etObj,manager,passnoworktime){
...
function getendtime(returnvalue,bObj,eObj,btObj,etObj,manager,passnoworktime){
	if(window.console) console.log(returnvalue,bObj.val(),eObj.val(),btObj.val(),etObj.val(),manager,passnoworktime);
...
function onFlownoShowDate_fnaDateEvent(inputname){
	try{
		...
		try{
			...
			if(inputnameArray.length==2&&inputnameArray[0]=="budgetperiod"){
				if(inputValue!=""){
        ...
        wfbrowvaluechange(_obj, _iptFieldid, _rowIndex);
	}catch(ex012){}
}
